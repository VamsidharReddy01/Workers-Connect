...
class _WorkerProfileSetupScreenState extends State<WorkerProfileSetupScreen> {
  ...
  Future<void> _pickPortfolioImages() async {
    final totalCount =
        _existingImages
            .where((img) => !_removedExistingIds.contains(img.id))
            .length +
  ...
  Future<void> _handleSubmit() async {
    ...
    if (_pendingImages.isNotEmpty) {
      final uploaded = await workerService.uploadWorkImages(
        _pendingImages
            .map(
              (img) =>
                  PortfolioUploadFile(filename: img.filename, bytes: img.bytes),
            )
            .toList(),
      );
    ...
    if (_isEditing) {
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(builder: (_) => WorkerNavigation(user: widget.user)),
      );
    }
  }
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildTextField(
                    ...
                    hintText:
                        'Describe your skills, specialties, and experience...',
